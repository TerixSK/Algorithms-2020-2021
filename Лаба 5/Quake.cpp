...
struct Label // Создаём структуру для наших меток, которая содердит 
{
...
int main()
{
    ...
    vector <string> a;
    vector <Label> label; // создаём метку
    queue < > Queue; // подрубаем очередь
    unsigned short int registers[26]; // подрубаем регистр
    for (int i = 0; i < 26; i++)
        registers[i] = 0; // присваеваем всем буквам регистр ноль
    while (!fin.eof()) // запускаем цикл пока не кончиться кончится файл
    {
        string com;
        fin >> com;
        a.push_back(com); // заряжаем нашм команды в строку
        if (com[0] == ':')
        {
            string name = com;
    ...
    while (i < a.size()) // запускаем цикл пока не пройдёмся по всем командам
    {
        ...
        else if (a[i][0] == 'J') // эта команда осуществляет переход на строку с меткой
        {
            ...
            for (int j = 0; j < label.size(); j++) // ищем в метках нужную
                if (label[j].name == str)
                {
                    i = label[j].num; // как только находим счетчик цикла становиться равен индексу метки 
